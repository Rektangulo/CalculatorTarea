package com.fernandez_martinez_daniel.calculator;
...
public class Calculator {
     ...
     * @return the ventana
     */
    public JFrame operativa_ventana() {
        return ventana;
    }
    ...
    */
    ...
    public Calculator() {
        ...
        btnMod = initBtn("%", x[2], y[1], event -> {
            ...
            if (Pattern.matches("([-]?\\d+[.]\\d*)|(\\d+)", inText.getText()))
                if (go) {
                    val = calc(val, inText.getText(), opt, 1);
    ...
    private Consumer<ItemEvent> calcTypeSwitchEventConsumer = event -> {
        ...
        switch (selectedItem) {
            case "Standard":
                operativa_ventana().setSize(WINDOW_WIDTH, WINDOW_HEIGHT);
                getBtnRoot().setVisible(false);
                getBtnPower().setVisible(false);
                getBtnLog().setVisible(false);
                break;
            case "Scientific":
                operativa_ventana().setSize(WINDOW_WIDTH + 80, WINDOW_HEIGHT);
